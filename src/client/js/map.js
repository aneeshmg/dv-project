...
function plot_business(business){
	...
	var infowindow = new google.maps.InfoWindow({content: biz_info});
	marker.addListener('mouseover', function() {infowindow.open(mapa, this);});
...
function plot_business_within_bound(mapa,business){
	...
	var competitors;
	$.ajax({'async': false,'type': "GET",'global': false,'url': "http://localhost:4000/getBusinessesWithinLoc/"+minLat+"/"+minLng+"/"+maxLat+"/"+maxLng,'success': function (data) {competitors = data;}});
	...
	for(i=0;i<Math.min(50,Object.keys(competitors).length);i++){
		...
		var marker = new google.maps.Marker(
			...
			position: new google.maps.LatLng(competitors[i].latitude,competitors[i].longitude),map: mapa,
		...
		infowindow = new google.maps.InfoWindow({content: competitor_info});
		marker.addListener('mouseover', function() {infowindow.open(mapa, this);});
		marker.addListener('mouseout', function() {infowindow.close();});
		marker.addListener('click', function() {
          mapa.setCenter(marker.getPosition());
